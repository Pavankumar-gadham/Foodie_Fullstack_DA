...
class RecipeListCreateView(generics.ListCreateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            print(f"Cache unavailable on GET: {e}", flush=True)
        ...
        try:
            ...
            print(f"Failed to cache on SET: {e}", flush=True)
...
class PurchasedRecipesView(generics.ListAPIView):
    serializer_class = PurchasedRecipeSerializer
    permission_classes = [permissions.IsAuthenticated]

    def get_queryset(self):
        return PurchasedRecipe.objects.filter(user=self.request.user)
